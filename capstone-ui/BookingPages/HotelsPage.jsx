...
export default function HotelsPage({ arrivalDate, departureDate,
                                     ...
                                     destID, cost, setCost,
                                     itinerary, setItinerary, filterActivities
                                    }){
    ...
    return (
        ...
        <div className="flex w-screen h-screen px-56 bg-slate-900">
            <div className="relative shadow-lg  py-4 px-8 bg-white  w-screen overflow-y-scroll">
                ...
                <div><div>Excluding taxes and fees.</div>
                <div><button disabled={Object.keys(itinerary).length === 0 ? true : false}
                     onClick={() => {filterActivities ? 
                                     navigate('/activities') :
                                     navigate('/booking')}}
                     className={Object.keys(itinerary).length === 0 ? 
                                `bg-gray-100 text-gray-400` : ``}
                     >{Object.keys(itinerary).length === 0 ? 
                       'Select a hotel to continue' : 'Continue'}</button></div>
                </div>
